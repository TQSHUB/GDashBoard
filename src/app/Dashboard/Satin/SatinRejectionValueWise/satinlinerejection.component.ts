...
import { SatinRejectionValueService } from '../../../Services/Satin/satinlinerejection.service';
...
export class SatinRejectionValueComponet{
 ...
 avgWarPageValue = 0;
   ...
   constructor(private satinlinerejectionService: SatinRejectionValueService, private datepipe: DatePipe, private searchPipe: SearchPipeSRVal, private jsondate: JsonDate){}
    ...
    script.src = '../../assets/ComponentJs/Satin/satinlinerejection.componet.js';
...
search()
    ...
    //   var ToDate = $("input[name=ToDate]").val();  
     ...
     if($("input[name=ToDate]").val() == ''){
       ...
       if(Type_string == 'NULL')
        ...
        this.busy = this.satinlinerejectionService.getSatinLineRejectionGrid(this.FromDate, this.ToDate, alias_string, Top_string, Type_string, orderby_string,orderbyA_string).subscribe(res => {
 ...
 ExportToExcel(){
       var date= new Date();
       ETE('SatinLineRejection_'+ this.datepipe.transform(date,'dd/MM/yyyy') +'.xls');
   }

  getBindItems_ByAliasName(){
    this.satinlinerejectionService.getBindItems_ByAliasName().subscribe(res => {
